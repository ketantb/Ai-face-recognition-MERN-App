...
router.post('/dashboard-details', async (req, resp) => {
    try {
        ...
        resp.json({ success: true, message: 'Your details saved successfully', newEntry: newEntry })
    }
    catch (err) {
...
router.put('/upload-images/:id', async (req, resp) => {
    ...
    try {
        ...
        if (existingData) {
            ...
            resp.json({ success: true, message: 'images uploaded successfully', data: existingData })
        }
        else {
